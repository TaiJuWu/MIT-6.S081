...
static void
kern_pagetable_init(pagetable_t kern_pagetable)
{
  // uart registers
  kvmmap(kern_pagetable, UART0, UART0, PGSIZE, PTE_R | PTE_W);
  ...
  kvmmap(kern_pagetable, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  kvmmap(kern_pagetable, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);

  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  kvmmap(kern_pagetable, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
}
...
kvminit()
{
  ...
  kern_pagetable_init(kernel_pagetable);
}
...
void
kvm_switch_pagetable(uint64 *addr)
{
...
void
kvmmap(pagetable_t kern_pagetable, uint64 va, uint64 pa, uint64 sz, int perm)
{
  if(mappages(kern_pagetable, va, sz, pa, perm) != 0)
    panic("kvmmap");
}
...
uint64
kvmpa(pagetable_t kern_pagetable, uint64 va)
{
  uint64 off = va % PGSIZE;
  pte_t *pte;
  uint64 pa;

  pte = walk(kern_pagetable, va, 0);
  if(pte == 0)
    panic("kvmpa");
  if((*pte & PTE_V) == 0)
    panic("kvmpa");
...
vmprint(pagetable_t pagetable)
{
  ...
  for(int i = 0; i < 512; i++){
    ...
    pagetable_t pagetable_1 = (pagetable_t)PTE2PA(pte);
    printf("..%d: pte %p pa %p\n", i, pte, pagetable_1);
    
    for(int j = 0; j < 512; ++j){
      pte_t pte = pagetable_1[j];
      if(!(pte & PTE_V)) continue;
      pagetable_t pagetable_2 = (pagetable_t)PTE2PA(pte);
      printf(".. ..%d: pte %p pa %p\n", j, pte, pagetable_2);

      for(int k = 0; k < 512; ++k){
        pte_t pte = pagetable_2[k];
        if(!(pte & PTE_V)) continue;
...
kvmcreate()
{
  pagetable_t kern_pagetable = uvmcreate();
  if(kern_pagetable == 0) {
    panic("kvmcreate");
  }
  kern_pagetable_init(kern_pagetable);
